...
mod folder;
use folder::folder::{json_file_search_uuid, read_json_file, Folder, BookData};

mod pdf;
use pdf::pdf::get_amazon_img;
...
async fn reg_pdf(axumPath(id): axumPath<String>, mut multipart: Multipart) -> String {
    ...
    let keep_path = Path::new(&path);
    if !keep_path.exists() {
        match fs::create_dir_all(keep_path) {
            Ok(_) => print!("directory がつくられました。"),
            Err(e) => {
                println!("directoryの作成に失敗 {}", e);
    ...
    let join = keep_path.join(&new_uuid);
    if !join.exists() {
        match fs::create_dir_all(&join) {
            Ok(_) => print!("directory がつくられました。"),
            Err(e) => {
                println!("directoryの作成に失敗 {}", e);
                return "err".to_string();
    ...
    while let Some(mut field) = multipart.next_field().await.unwrap() {
        if field.name() == Some("textData") {
            let text_data = field.text().await;
            if let Ok(st) = text_data {
                let (img_name, title) = get_amazon_img(&st, &join).await.unwrap();
                img_name_path = img_name;
                pdf_title = title;
            }
    ...
    let mut json = File::create(&join.join("meta.json")).await.unwrap();
    let mata_json = BookData {
        id: new_uuid,
        img: img_name_path,
        title: pdf_title,
    };
